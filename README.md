# JustDeco
 A simple mod for 7 days to die: Just deco!

# Purpose
This mod allows to craft some items from vanilla game by a normal player. Some of those items are sellable in traders and most of them can be created in Creative Mode but they are not being able to be crafted. So, this mod could help to those players who like to build inmersive construction.

It can be used as a server mod without problems because only adds some recipes and modify some blocks.

It is not purpose of the mod to impact in the game but help players in their decorations.

# Compatibility

Currently it is compatible with **game version 2.6**. Probably it can work with older versions but not guaranteed, it will be compatible if those versions got the items already because all of the items included in this mod come from the vanilla game.


# What's new
## Version 1.0.0
- Compatible with game version 2.6
- New recipes:
  - Bandit wall sign
  - Church pew pile
  - 3 driftwoods
  - 3 firewoods stacks
  - 3 horse shoes
  - 4 utility poles
  - 5 wild west coffins
 
  
# Included recipes

## **Outdoor furniture**

- 8 ember piles. Fixed the economic value to something useful.
 
![image](https://github.com/user-attachments/assets/abfd144b-0e8d-4cee-b4c3-8ec97dc39381)
 
- 22 table umbrellas (11 colours and two variants, up and down umbrella). 
 
![image](https://github.com/user-attachments/assets/0104387b-c0f0-4016-bd46-3c889bc016f6)

- 10 camping chairs (10 colours). Fixed the economic value to something useful.

![image](https://github.com/user-attachments/assets/1a4f06f5-adff-4ce1-b140-b59904b3e95d)

- 2 park benches. Fixed the economic value to something useful.

![ApplicationFrameHost_mCEHzJq5dL](https://github.com/user-attachments/assets/91f14060-aa11-4694-8709-c1a576efd1e5)

- 3 Playground equipment. Fixed the economic value to something useful.

![ApplicationFrameHost_1UMGKYwlqg](https://github.com/user-attachments/assets/a0117975-3822-4b29-b574-7db46ef2c131)

- 4 Hanging plants (2 ivys, 1 moss and 1 cobweb). Removed the economic value and now they are not sellable to trader.

![image](https://github.com/user-attachments/assets/5086c8c0-f6ff-47fa-9ab8-f8b580ca6d49)

- 3 Satellite dishes. 

![ApplicationFrameHost_3H3MIx79Zt](https://github.com/user-attachments/assets/f6045acf-2764-4fac-971e-b690ad97e31a)

- 8 gravestones. Removed the economic value and now they are not sellable to trader.

![ApplicationFrameHost_n5IwO3eef5](https://github.com/user-attachments/assets/ab99ecf7-c427-418d-87cb-251c4ecbe786)

- 1 Construction Cone. 

![ApplicationFrameHost_30yNPFKMLv](https://github.com/user-attachments/assets/5bf008df-a526-401e-a904-f44bbf36356b)

- 10 Bandit platforms. 

![ShareX_1jqZMKkVJq](https://github.com/user-attachments/assets/c207ea78-5488-4d0a-b931-dd70ae8be39d)

- 11 Bandit supports. 

![ApplicationFrameHost_stm0ZGmlpG](https://github.com/user-attachments/assets/1f28c7a1-2803-4041-847b-e6396b0f18d9)

- 6 Bandit ramps.
  
![ApplicationFrameHost_JEVSnVlBHX](https://github.com/user-attachments/assets/4bdc1a5a-9231-47ce-bc7f-f09d15bc9ed1)

- Wild West Wagon. 

![image](https://github.com/user-attachments/assets/17d81524-972a-4ae2-8346-1ee5dbfeb83d)

- 7 items related to liftment (hoist). Fixed the economic value to something useful.

![7DaysToDie_3nlQXYFlYW](https://github.com/user-attachments/assets/965bc1dd-b28d-4769-8f5a-7f890fee5bd9)

- Fancy Garbage can. Added to Metal Furniture block

![ApplicationFrameHost_qfOp1A14Gx](https://github.com/user-attachments/assets/8cee2c80-4df0-422b-b9f0-a8593cede333)

- 2 Trash cans ("Brute" Style). 

![7DaysToDie_r43TDplOhP](https://github.com/user-attachments/assets/6809eb49-b3d1-4186-a26b-344e61ff82d5)

- 3 Flag Poles. Their economic value has been fixed and they are not sellable to trader.

![7DaysToDie_vo2jdkpWtY](https://github.com/user-attachments/assets/7e6153e3-f97b-48fb-be0d-9b3185778278)

- Pig trough (just the empty version). 

![7DaysToDie_bkMYSKhwuV](https://github.com/user-attachments/assets/4637ff71-165e-4b5c-8265-04f4c02665c7)

- 5 modular ropes (some of them are climbable).

![7DaysToDie_4HXC9qoCGk](https://github.com/user-attachments/assets/fd2515bf-3f6e-429a-b260-6e50ad9c8217)

- 36 camping tents. They are 6 different types with 6 colours each one (White, Red, Orange, Green, Blue and Black)

![WhiteTents](https://github.com/user-attachments/assets/3d7b1108-1890-46b7-9fc2-91fe22f06b1f)

![Tents](https://github.com/user-attachments/assets/1edf04cc-b090-4acb-84d5-f61c5d30cab3)

- 24 tarp fences. They are 6 different types with 4 colours each one (White, Red, Green and Blue)

![TarpFences](https://github.com/user-attachments/assets/c0e54fca-61dd-4518-9140-1c6657180069)

- 9 Corrugated Bandit Walls. 

![BanditWallsCorrugated](https://github.com/user-attachments/assets/8117806d-dbe1-48f4-8a00-198edb439994)

- 56 Bandit Walls. Available colours: White (in the following screenshot), Brown, Red, Orange, Yellow, Green, Blue and Grey.

![BanditWallsWhite](https://github.com/user-attachments/assets/6c7f99f2-9df4-4cb2-920c-d596f39c1409)

- 5 Pallet Boxes. They have been fixed so they are not sellable to trader now.

![PalletBoxes](https://github.com/user-attachments/assets/df6907b6-e8d8-4dc5-893f-6c6814edb815)

- 24 Pallet Bricks. They are from vanilla game (6 types and four colours: Grey, Brown, Red and Orange). They have been fixed so they are not sellable to trader now.

![PalletBricks](https://github.com/user-attachments/assets/9922a24c-10c7-4e2c-9fbd-57d90f54eeda)

- 24 Pallet Tarps. They are from vanilla game (2 types and 12 colours: White, Brown, Red, Orange, Yellow, Green, Blue, Purple, Grey, Black, Pink and Army Green). They have been fixed so they are not sellable to trader now.

![PalletTarps](https://github.com/user-attachments/assets/8e96e0df-14a5-4efc-b5f2-93cecaab5fea)

- 12 Misc pallets. They have been fixed so they are not sellable to trader now.

![MiscPallets](https://github.com/user-attachments/assets/09361ae8-0c2a-46d1-bb0a-0309825ba4fd)

- 7 cow pallets. They have been fixed so they are not sellable to trader now.

![PalletsCowFeed](https://github.com/user-attachments/assets/2c1b92cc-3cc4-4db4-8353-b9052089d3e1)

- 4 military pallets. They have been fixed so they are not sellable to trader now.

![PalletsMilitary](https://github.com/user-attachments/assets/a3d0d3ee-9907-4fa6-bcec-aa24fd6fac43)

- 7 fluor pallets. They have been fixed so they are not sellable to trader now.

![PalletsFluor](https://github.com/user-attachments/assets/f491bff1-237b-43f0-80ff-93e290dafac3)

- 7 sand pallets. They have been fixed so they are not sellable to trader now.

![PalletsSand](https://github.com/user-attachments/assets/18c3c0a4-7079-45ec-92d1-71bc96a737a8)

- 8 concrete pallets. They have been fixed so they are not sellable to trader now.

![PalletsConcrete](https://github.com/user-attachments/assets/89026daa-a435-48fa-88c9-c6466a85b1f9)

- 7 fertilizier pallets. They have been fixed so they are not sellable to trader now.

![PalletsFertilizer](https://github.com/user-attachments/assets/c8a288e8-a68f-450a-a609-43d343fa9433)

- 84 bulk goods pallets. They have been fixed so they are not sellable to trader now. There are 7 types and 12 colours: White (in the following image), Brown, Red, Orange, Yellow, Green, Blue, Purple, Grey, Black, Pink and Army Green.

![PalletBulkGoods](https://github.com/user-attachments/assets/81441b0a-0804-474a-9ba7-213f0cff30d0)

- 12 rubbish decorations.

![RubbishDecor](https://github.com/user-attachments/assets/428ad1b3-fb4b-4c51-9ecd-33234edcce52)

- 4 blood decorations. Economic value have been fixed so they are not sellable to trader now. Crafting time reduced to 5 seconds.

![7DaysToDie_7nAoN4anBz](https://github.com/user-attachments/assets/f73a58e6-d0f6-4eaf-a52a-2118ae10a181)

- 1 Wall Sign. 

<img width="112" height="77" alt="BanditWallSignInGame" src="https://github.com/user-attachments/assets/b7d75361-753c-458e-a3c5-740f1b47bfe0" />

 - 3 firewoods stacks. Economic value have been fixed.

<img width="242" height="78" alt="FirewoodStacks" src="https://github.com/user-attachments/assets/3963cdcb-eca8-41e2-b606-edda8588e2a5" />

- 4 utility poles. Economic value have been fixed.

<img width="294" height="72" alt="UtilityPoles" src="https://github.com/user-attachments/assets/1f69d2ae-4518-473c-a2fa-ea4b231b6f3b" />

- 3 driftwoods. Economic value have been fixed.

<img width="245" height="83" alt="Driftwoods" src="https://github.com/user-attachments/assets/15623192-0851-4660-90ce-fd2eb216d2bb" />

## **Indoor furniture**

- 6 Beds. Decreased the economic value to something useful and now they are not sellable to trader.

![image](https://github.com/user-attachments/assets/93566bfe-1905-4340-a474-a1acd34f4035)

- 4 Newspapers deco. 

![image](https://github.com/user-attachments/assets/064ecf46-8295-474e-9e33-d6c96c77ff4a)

- 5 Wine Bottles deco. 

![ShareX_HlbfACHBLz](https://github.com/user-attachments/assets/6d2bc824-28ca-4d64-bffc-4f12dc7cddcb)

- 2 Tubs. Claw foot tub economic value has been fixed to be similar to Empty Bathtub

![7DaysToDie_dIvnCpduST](https://github.com/user-attachments/assets/4e44b9dd-1d5e-41e6-8c7f-f85197d09f74)

- Piano.

![image](https://github.com/user-attachments/assets/36ebdb46-7012-4453-9bef-624e0d576bba)

- Radio station. Its economic value has been fixed.

![image](https://github.com/user-attachments/assets/5e9b7d8a-f724-478d-8123-1d06c0f361da)

- 4 church bells. 

![image](https://github.com/user-attachments/assets/88d8e462-9b6d-4931-a775-07d6b0cb95e1)

- 3 hedges. 

![7DaysToDie_TY0wRdy9f2](https://github.com/user-attachments/assets/226c45c3-112f-43f7-9de6-b02cf5d20d4f)

- Curtain rod. 
  
 ![CurtainRod](https://github.com/user-attachments/assets/9420de50-8b5f-44bf-8db6-959f486111cf)


- 4 Curtain Dividers. 

 ![Divider](https://github.com/user-attachments/assets/9f10e685-4309-4dc8-bf39-7bdbe4b5910e)


- 18 Curtains per colour.  Available colours: White (in the following screenshot), Brown, Red, Orange, Yellow, Blue, Pink, Purple, Grey and Black.

![Curtains](https://github.com/user-attachments/assets/8fcb4b0e-9d2d-44c2-9f6b-a58ef124fe2a)

- 5 empty shelves. They are added to metal furniture block.

![image](https://github.com/user-attachments/assets/024250b9-6f83-406d-8215-4765e18fff41)

- 3 shower head and handle. 

![image](https://github.com/user-attachments/assets/de79083e-6f95-485a-af05-8fbcf7f5e339)

- Church pew pile. It is from vanilla game, only found in Jen's prefab. Its economic value has been fixed and it is not sellable to trader.

<img width="112" height="68" alt="ChurchPewPile" src="https://github.com/user-attachments/assets/95ddce45-61f0-45af-865f-e100900e6824" />

- 5 wild west coffins. Their economic value have been fixed and they are not sellable to trader.

<img width="388" height="77" alt="WildWestCoffins" src="https://github.com/user-attachments/assets/3f684d2f-00c5-4101-bdd2-95fd77e79a9f" />

- 3 horse shoes. 

<img width="246" height="82" alt="HorseShoes" src="https://github.com/user-attachments/assets/07a2b536-4406-4cd0-81bc-a06067ef0ad5" />

# How to install the mod

- Download mod. You can find the latest release in this repository.
- In Steam, right-click on 7 Days to Die, choose Properties
- Click on the Local Files tab and click Browse Local Files
- Open the folder named Mods (create if if it does not exist)
- Unzip the file into that Mods folder. You will see a new folder with some files and a subfolder named Config.

# Disclaimer

This mod has been created specifically to my own use. So their content could be biased by my style and way of play. Feel free to suggest new ideas and content but be brunderstanding with it!
